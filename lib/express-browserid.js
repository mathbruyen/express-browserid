            ...
            vres.on('data', function (chunk) { body += chunk; } )
                .on('end', function () {
                    try {
                        ...
                        if (valid) {
                            getEmailFilter(opts)(verifierResp.email, function(){
                                if (req.session) req.session[getSessionKey(opts)] = email;
                                res.json({ status: "okay", email: email })
                            }, function(reasonMessage) {
                                res.json({ status: "error", reason: reasonMessage });
                            });
                        } else {
                            res.json({ status: "error", reason: verifierResp.reason });
                        }
                    }
                    catch (e) {
...
function getEmailFilter(opts) {
    // Accept everything by default
    return opts.emailFilter || function(email, valid, invalid) { valid(); };
}
